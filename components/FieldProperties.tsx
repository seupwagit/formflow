...
interface FieldPropertiesProps {
  ...
  onClose: () => void
  allFields?: FormField[] // Para verificar nomes únicos
...
export default function FieldProperties({ 
  ...
  // 🔧 Função inteligente para determinar se deve mostrar correção de nome
  const shouldShowNameCorrection = (field: FormField): boolean => {
    if (!field || !field.name || !field.id) return false
    
    // 1. Se o nome já está em formato válido snake_case, NÃO mostrar correção
    const hasValidFormat = /^[a-z][a-z0-9_]*$/.test(field.name)
    const isReasonableLength = field.name.length <= 50
    
    // 2. Se o nome foi sanitizado corretamente do label, NÃO mostrar correção
    ...
    // 3. Se o nome não tem caracteres problemáticos, NÃO mostrar correção
    const hasProblematicChars = /[^a-z0-9_]/.test(field.name)
    
    // 4. Se o campo foi corrigido recentemente (nome válido), NÃO mostrar
    if (hasValidFormat && isReasonableLength && !hasProblematicChars) {
      console.log('✅ Campo com nome válido, não mostrando correção:', field.name)
      return false
    }
    
    // 5. Verificar se é um ID muito longo gerado automaticamente
    const isAutoGeneratedId = field.name.includes('_') && field.name.length > 30
    const hasTimestamp = /\d{6,}/.test(field.name)
    
    // Só mostrar correção se há problemas REAIS
    const shouldShow = hasProblematicChars || (isAutoGeneratedId && hasTimestamp)
    
    console.log('🔍 Verificação de correção:', {
  ...
  const validateAndUpdateFieldName = (name: string) => {
    ...
    if (!isUnique) {
      ...
      validation.errors.push('Nome já existe em outro campo')
      validation.suggestions.push('Escolha um nome único')
    }
  ...
  }, [field?.id, field?.name]) // Incluir field.name para reagir a mudanças

  // Validação separada quando allFields muda
  useEffect(() => {
    if (editedField?.name) {
      validateAndUpdateFieldName(editedField.name)
    }
  }, [allFields.length]) // Apenas quando o número de campos muda
  ...
  const handleSave = () => {
    ...
    console.log('💾 Salvando propriedades do campo:', updatedField)
  ...
  const handleFieldChange = (key: keyof FormField, value: any) => {
    ...
    // Nome do campo não é mais editável - ignorar mudanças no nome
    if (key === 'name') {
      return
    }
  ...
  const fieldTypes = [
    ...
    { value: 'select', label: 'Lista de Seleção', icon: '📋' },
    { value: 'dynamic_list', label: 'Lista Dinâmica', icon: '🔗' },
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="relative">
                {/* Campo SEMPRE não editável para evitar quebras no sistema */}
              ...
              <div className="flex items-center justify-between mt-2">
                <p className="text-xs text-green-600 flex items-center space-x-1">
                  ...
                  <span>Nome fixo na tabela PostgreSQL - não editável para evitar quebras</span>
                </p>
              </div>

              {/* Botão de correção forçada - só mostrar se realmente há problema */}
            ...
            <div>
              ...
              <input
                ...
                placeholder="Texto exibido para o usuário final"
              />
              <p className="text-xs text-gray-500 mt-1">
                Texto exibido para o usuário final
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              />
              <p className="text-xs text-gray-500 mt-1">
                💡 No modo canvas, se vazio, não exibe nada dentro do campo
              </p>
            </div>
            ...
            <div>
              ...
              />
              <p className="text-xs text-gray-500 mt-1">
                📝 Valor que aparece automaticamente quando o formulário é aberto
              </p>
            </div>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center space-x-2">
              ...
              <span>Dicas / Texto de Ajuda</span>
            </label>
            <textarea
          ...
          {/* Configuração para Lista Dinâmica */}
          ...
          {/* Configuração para Campo Calculado */}
          {editedField.type === 'calculated' && (
            <CalculatedFieldConfig
              ...
              onUpdate={(updates) => {
                ...
                // IMPORTANTE: Salvar no banco através do onFieldUpdate
                onFieldUpdate(updatedField)
                if (showSuccess) {
                  showSuccess('Campo Calculado', 'Configuração salva com sucesso!')
                }
              }}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ex: 0"
                  />
                  <p className="text-xs text-gray-500 mt-1">✅ Aceita zero</p>
                </div>
                <div>
                  <label className="block text-xs text-gray-600 mb-1">
                    Valor Máximo
          ...
          <div>
            ...
            {showAdvanced && (
              <div className="mt-4 p-4 bg-gray-50 rounded-lg space-y-4">
                ...
                {/* Controles de Alinhamento do Conteúdo */}
                <div className="border-t pt-4">
                  <h4 className="text-sm font-medium text-gray-700 mb-3">Alinhamento do Conteúdo</h4>
                  ...
                  <p className="text-xs text-gray-500 mt-2">
                    Define como o texto será posicionado dentro do campo no formulário e no PDF
                  </p>
                </div>

                {/* Controles de Formatação de Fonte */}
                <div className="border-t pt-4">
                  <h4 className="text-sm font-medium text-gray-700 mb-3">Formatação de Fonte</h4>
                  
                  <div className="grid grid-cols-2 gap-4">
                    {/* Família e Tamanho */}
                    <div>
                      <label className="block text-xs text-gray-600 mb-2">Família da Fonte</label>
                      <select
                  ...
                  <div className="grid grid-cols-3 gap-4 mt-3">
                    ...
                    {/* Decoração */}
                    <div>
                      <label className="block text-xs text-gray-600 mb-2">Decoração</label>
                      <select
                  ...
                  <p className="text-xs text-gray-500 mt-2">
                    Define a aparência do texto no formulário e no PDF
                  </p>
                </div>
              </div>
            )}
